...
layout(input_attachment_index=4, set=_DS_SMP_DFR_COLOR, binding=0) uniform subpassInput colorInput;
...
vec3 blinnPhongPositional(vec3 pos, vec3 norm, vec3 diff) {
    vec3 diffuse = vec3(0.0);
    for (int i = 0; i < lgtPos.length(); i++) {
        if ((lgtPos[i].flags & LIGHT_SHOW) > 0) {
            vec3 s = normalize(vec3(lgtPos[i].position) - pos);
            float sDotN = max(dot(s, norm), 0.0);
            diffuse += lgtPos[i].L * diff * sDotN;
        }
    }
    return diffuse;
}
...
// vec3 blinnPhongSpot(vec3 pos, vec3 norm, vec3 diff) {
...
//                 return lgtSpot[i].La + spotScale * lgtSpot[i].L * (diff + spec);
//             }
...
void main() {
    ...
    vec3 norm = subpassLoad(normInput).xyz;
    vec4 diff = subpassLoad(colorInput);

    outColor = vec4(
        blinnPhongPositional(pos, norm, diff.rgb)
        // + blinnPhongSpot(pos, norm, diff.rgb)
        ,diff.a
        );
}