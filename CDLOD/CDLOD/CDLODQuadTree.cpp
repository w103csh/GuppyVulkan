...
CDLODQuadTree::Node::LODSelectResult CDLODQuadTree::Node::LODSelect(LODSelectInfo &lodSelectInfo,
    ...
    if (!(bRemoveSubTL && bRemoveSubTR && bRemoveSubBL && bRemoveSubBR) &&
        ...
        int LODLevel = lodSelectInfo.StopAtLevel - this->GetLevel();  // The LOD level is inverted here... CH
...
//
void CDLODQuadTree::LODSelection::GetMorphConsts(int LODLevel, glm::vec4 &data2) const {
    ...
    //
    data2[0] = mStart;
    data2[1] = 1.0f / (mEnd - mStart);
    //
    data2[2] = mEnd / (mEnd - mStart);
    // consts[3] = 1.0f / (mEnd - mStart); // This is the same as [1]? Right?
}
//
