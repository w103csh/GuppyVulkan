...
void Uniform::Base::init(const VkDevice &dev, const Game::Settings settings) {
    // TODO: allocate buffer(s) on the handler and just set the "info" object values here!!!!
    ...
    //   resource_.info.range = helpers::createBuffer(dev, resource_.info.range,
    // VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, ctx,
    // resource_.info.buffer, resource_.memory);

    //   if (settings.enable_debug_markers) {
    //       std::string markerName = NAME + " block";
    //       ext::DebugMarkerSetObjectName(dev, (uint64_t)resource_.info.buffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT,
    //                                     markerName.c_str());
    //       // todo: what should this be???
    //       ext::DebugMarkerSetObjectTag(dev, (uint64_t)resource_.info.buffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, 0,
    //                                    sizeof(_UniformTag), &tag);
    //   }
}

VkDescriptorSetLayoutBinding Uniform::Base::getDecriptorLayoutBinding(const VkShaderStageFlags &stageFlags,
                                                                      const uint32_t &binding, uint32_t descriptorCount) {
    VkDescriptorSetLayoutBinding layoutBinding = {};
    layoutBinding.binding = binding;
    layoutBinding.descriptorType = descriptorType_;
...
void Uniform::Base::destroy(const VkDevice &dev) {
    if (resource_.info.buffer != VK_NULL_HANDLE) vkDestroyBuffer(dev, resource_.info.buffer, nullptr);
    if (resource_.memory != VK_NULL_HANDLE) vkFreeMemory(dev, resource_.memory, nullptr);
}

VkWriteDescriptorSet Uniform::Base::getWrite(const uint32_t &dstBinding, uint32_t dstArrayElement,
                                             uint32_t descriptorCount) {
    VkWriteDescriptorSet write = {};
    write.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    write.dstBinding = dstBinding;
    write.dstArrayElement = dstArrayElement;
    write.descriptorType = descriptorType_;
    write.pBufferInfo = &resource_.info;
