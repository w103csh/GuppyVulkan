...
//#include "Plane.h"
//
void GuppyApp::run(int argc, char* argv[]) {
    // initWindow();
    // initVulkan();
    // mainLoop();
    // cleanup();
...
void GuppyApp::init_vulkan(struct sample_info& info) {
    VkResult U_ASSERT_ONLY res = {};
    const bool depthPresent = true;
...
// void GuppyApp::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer,
...
//
//// this all all bad
// void GuppyApp::flushSetupCommandBuffers(const VkCommandPool commandPool, const VkQueue queue, const std::function<void()>&
// lambda)
//{
...
//}
//
// VkFormat GuppyApp::findSupportedFormat(const std::vector<VkFormat>& candidates, const VkImageTiling tiling, const
