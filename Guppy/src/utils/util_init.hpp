...
void init_descriptor_set_layout(struct sample_info &info, const VkDescriptorSetLayoutCreateFlags descSetLayoutCreateFlags = 0);
void init_pipelineLayout(struct sample_info &info, const VkDescriptorSetLayoutCreateFlags descSetLayoutCreateFlags = 0);
void init_renderpass(struct sample_info &info, bool include_depth, bool include_color = true, bool clear = true,
                     VkImageLayout finalLayout = VK_IMAGE_LAYOUT_PRESENT_SRC_KHR);
void init_vertexBuffer(struct sample_info &info, const void *vertexData, uint32_t dataSize, uint32_t dataStride, bool use_texture);
void init_framebuffers(struct sample_info &info, bool include_depth = true, bool include_color = true);
