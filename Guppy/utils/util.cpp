...
void create_image(struct sample_info &info, const VkSampleCountFlagBits num_samples, const VkFormat format,
                  const VkImageTiling tiling, const VkImageUsageFlags usage, VkFlags requirements_mask, VkImage &image,
    ...
    image_info.extent.width = info.swapchain_extent.width;
    image_info.extent.height = info.swapchain_extent.height;
    ...
    pass =
        memory_type_from_properties(info, mem_reqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &alloc_info.memoryTypeIndex);
    assert(pass);
...
VkImageView create_image_view(const VkDevice &device, const VkImage& image, const uint32_t mipLevels, const VkFormat format,
...
void set_image_layout(struct sample_info &info, VkCommandBuffer& cmd, VkImage image, VkImageAspectFlags aspectMask, VkImageLayout old_image_layout,
