...
VkResult init_enumerate_devices(struct sample_info &info, uint32_t gpu_count) {
    ...
    for (auto &device : gpus) {
        ...
        if (formatCount != 0) {
            props.surf_props.formats.resize(formatCount);
            vkGetPhysicalDeviceSurfaceFormatsKHR(props.device, info.surface, &formatCount, props.surf_props.formats.data());
        }
...
void init_depth_buffer(struct sample_info &info) {
    ...
    // TODO: move this into a determine function

    const VkFormat depth_format = info.depth.format;
    VkFormatProperties props;
    VkImageTiling tiling;
    vkGetPhysicalDeviceFormatProperties(info.physical_device_properties[0].device, depth_format, &props);
    ...
    // TODO: add getMaxUsableSampleCount to NUM_SAMPLES!
    create_image(info, NUM_SAMPLES, depth_format, tiling, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT,
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, info.depth.image, info.depth.mem);
    ...
    info.depth.view =
        create_image_view(info.device, info.depth.image, info.mip_levels, depth_format, aspectFlags, VK_IMAGE_VIEW_TYPE_2D);
}
...
bool determine_swapchain_support(struct sample_info &info, const physical_device_properties props) {
    ...
    return !props.surf_props.formats.empty() && !props.surf_props.presentModes.empty();
}
...
void determine_swapchain_surface_format(struct sample_info &info, const surface_properties& props) {
    ...
    // no preferred type
    if (props.formats.size() == 1 && props.formats[0].format == VK_FORMAT_UNDEFINED) {
        info.swapchain_format = {VK_FORMAT_B8G8R8A8_UNORM, VK_COLOR_SPACE_SRGB_NONLINEAR_KHR};
        return;
    } else {
        for (const auto &surfFormat : props.formats) {
    ...
    info.swapchain_format = props.formats[0];
}
...
void init_swapchain(struct sample_info &info, VkImageUsageFlags usageFlags) {
    ...
    for (uint32_t i = 0; i < info.swapchainImageCount; i++) {
        ...
        sc_buffer.view = create_image_view(info.device, sc_buffer.image, info.mip_levels, info.swapchain_format.format,
...
void init_renderpass(struct sample_info &info, bool include_depth, bool clear, VkImageLayout finalLayout) {
    ...
    attachments[0].format = info.swapchain_format.format;
